...
src/
  ...
  lib/                 # everything that is not a component
    ...
    docker.ts          # optional CPU/memory from the Docker socket
  ...
  app/api/             # auth, me, metrics, server, users, whitelist,
                       # console, logs, mods, modpack, backups
  app/(dash)/          # the eight pages
...
Every mount into the container is **read-only** except the database connection.
The panel can ask the server to change things over RCON, but it cannot write to
